...
void exitto(int (*entrypoint)())
{
	...
	{
		uintptr_t pa = gxbb_share_mem_in_base();
		uintptr_t sz = 0x1000;
		map_phys(PTE_XN | PTE_AF | PTE_SH_INNER | PTE_AP_KRWUNA | PTE_ATTR_NORMEM_UC, (caddr_t)(SEGKPM_BASE + pa), pa, sz);
	}
	{
		uintptr_t pa = gxbb_share_mem_out_base();
		uintptr_t sz = 0x1000;
		map_phys(PTE_XN | PTE_AF | PTE_SH_INNER | PTE_AP_KROUNA | PTE_ATTR_NORMEM_UC, (caddr_t)(SEGKPM_BASE + pa), pa, sz);
	}
