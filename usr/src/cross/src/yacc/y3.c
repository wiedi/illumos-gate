...
static void go2gen(int c)
{
	...
	if (g2debug && foutput != NULL) {
		(void) fprintf(foutput, WSFMT("%ls: gotos on "),
		    nontrst[c].name);
		NTLOOP(i) if (temp1[i])
			(void) fprintf(foutput, WSFMT("%ls "), nontrst[i].name);
		(void) fprintf(foutput, "\n");
	}
...
wdef(wchar_t *s, int n)
{
	...
	(void) fprintf(ftable, WSFMT("# define %ls %d\n"), s, n);
}
...
{
	int i;
	(void) fprintf(ftable, WSFMT("static YYCONST yytabelem %ls[]={\n"), s);
	for (i = 0; i < n; ) {
		if (i % 10 == 0)
			(void) fprintf(ftable, "\n");
