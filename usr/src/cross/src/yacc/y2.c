 ...
 */
		...
		if (t == L'=') {
			...
			if ((t = gettok()) == IDENTIFIER) {
				...
				lrprnt();		/* dump lhs, rhs */
				(void) swprintf(actname, sizeof(actname), L"$$%d", nprod);
				/*
				 * make it nonterminal
				 */
				j = chfind(1, actname);
...
defout()
{
	...
	for (i = ndefout; i <= ntokens; ++i) {
		...
		if (*cp == L' ')	/* literals */
		{
			(void) fprintf(fdebug, WSFMT("\t\"%ls\",\t%d,\n"),
 ...
 */
			 ...
			 */
			(void) fprintf(faction, WSFMT("$%ls"), tokname);
/*
 * TRANSLATION_NOTE  -- This is a message from yacc.
 *	This message is passed to warning() function.
 ...
 */
	...
	rhsfill((wchar_t *)0);
	(void) wcscpy(lhstext, s); /* don't worry about too long of a name */
}
 ...
 */
	...
	(void) fprintf(fdebug, WSFMT("\t\"%ls :%ls\",\n"), lhstext, rhs);
	if (m_rhs)
		free(m_rhs);
}
