 ...
 * (e.g. %ls) and misdiagnoses them as being malformed.
 ...
 * variable.
 *
 * When lint is able to handle %ls, it would be appropriate
...
start:
						...
						while(n){
							...
							*p++ = 0;
							...
							i = sptr*2;
							if(!ratfor)(void) fprintf(fout,"# ");
							(void) fprintf(fout,WSFMT("define %ls %d\n"),t,i);
							scopy(t,sp);
							sname[sptr] = sp;
				...
				case ' ': case '\t':		/* must be code */
					lgate();
					if( p[1]=='/' && p[2]=='*' ) cpycom(p);
					else (void) fprintf(fout, WSFMT("%ls\n"),p);
					continue;
