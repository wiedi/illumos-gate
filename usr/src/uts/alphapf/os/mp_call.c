...
void
cpu_call_nowait(cpu_t *cp, cpu_call_func_t func, uintptr_t arg1, uintptr_t arg2)
{
	cpuset_t set;
	...
	/*
	 * If we are on the target CPU, call the function directly, but raise
	 * the PIL to XC_PIL. This guarantees that functions called via
	 * cpu_call_nowait() can not ever interrupt each other.
	 */
	if (CPU == cp) {
		int save_spl = splr(ipltospl(XC_HI_PIL));
		...
		CPUSET_ONLY(set, cp->cpu_id);
		xc_call_nowait((xc_arg_t)arg1, (xc_arg_t)arg2, 0,
