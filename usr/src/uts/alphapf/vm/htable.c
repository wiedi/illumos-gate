		...
		if (!IN_VPT(base)) {
			...
			if (PTE_ISVALID(pte)) {
				...
				if (PTE_ISPAGE(pte, level)) {
					PTE_CLR(pte, PTE_SOFTWARE);
					PTE_SET(pte, PTE_ASM|PTE_NOCONSIST);
					ptep[i] = pte;
				} else {
...
htable_e2va(htable_t *ht, uint_t entry)
{
	...
	va = ((intptr_t)(((ht->ht_vaddr >> LEVEL_SHIFT(l)) + entry) << (LEVEL_SHIFT(l) + (64 - VA_BITS))) >> (64 - VA_BITS));
	return (va);
}
