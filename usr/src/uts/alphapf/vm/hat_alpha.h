...
/*
 * alpha specific routines for use online in setup or aarch64pf/vm files
 */
 ...
 */
extern void  hat_thread_switch(kthread_t *thd);
extern int hat_page_falt(hat_t *hat, caddr_t vaddr, int cause);
extern void hat_set_pcb(hat_t *hat, pcb_t *pcb);
extern uint64_t hat_getptbr(struct hat *hat);
#define IN_VPT(va)	((LEVEL_MASK(mmu.max_level) & (uintptr_t)(va)) == VPT_BASE)
