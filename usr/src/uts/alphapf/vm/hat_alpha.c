...
hat_alloc(struct as *as)
{
	...
	for (int r = 0; r < num_kernel_ranges; ++r) {
		...
		for (va = rp->hkr_start_va; va != rp->hkr_end_va;
			...
			if (rp->hkr_level == mmu.max_level)
				ht = hat->hat_htable;
			else
				ht = htable_create(hat, va, rp->hkr_level, NULL);

			start = htable_va2entry(va, ht);
			cnt = HTABLE_NUM_PTES(ht) - start;
			eva = va + ((uintptr_t)cnt << LEVEL_SHIFT(rp->hkr_level));

			if (rp->hkr_end_va != 0 && (eva > rp->hkr_end_va || eva == 0))
				cnt = htable_va2entry(rp->hkr_end_va, ht) - start;

			src = htable_lookup(kas.a_hat, va, rp->hkr_level);
			ASSERT(src != NULL);
			pte_copy(src, ht, start, cnt);
			htable_release(src);
		}
	}

	/*
	 * top page is self referrenced
	 */
	ht = hat->hat_htable;
	*PT_INDEX_PTR(hat_kpm_pfn2va(ht->ht_pfn), htable_va2entry(VPT_BASE, ht)) = MAKEPTP(ht->ht_pfn, mmu.max_level);
...
hat_init_finish(void)
{
	 ...
	 */
	NEXT_HKR(r, mmu.max_level, SEG1_BASE, 0);
	...
	for (r = 0; r < num_kernel_ranges; ++r) {
		rp = &kernel_ranges[r];
		for (va = rp->hkr_start_va; va != rp->hkr_end_va; va += LEVEL_SIZE(rp->hkr_level)) {
			htable_t *ht;
			...
			if ((ht = htable_lookup(kas.a_hat, va, rp->hkr_level - 1)) != NULL)
				htable_release(ht);
			else
				htable_create(kas.a_hat, va, rp->hkr_level - 1, NULL);
		}
	}
