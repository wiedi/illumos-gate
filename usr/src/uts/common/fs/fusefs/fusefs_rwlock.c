 ...
 */
int
fusefs_rw_enter_sig(fusefs_rwlock_t *l, krw_t rw, int intr)
{
	...
	if (l->owner == curthread) {
		...
		if ((l->count % 10000) == 9999)
			cmn_err(CE_WARN, "fusefs_rw_enter_sig: count %d on"
			    "rwlock @ %p\n", l->count, (void *)&l);
#endif
		l->count++;
	} else {
...
int
fusefs_rw_tryenter(fusefs_rwlock_t *l, krw_t rw)
{
	mutex_enter(&l->lock);
...
void
fusefs_rw_exit(fusefs_rwlock_t *l)
{
...
int
fusefs_rw_lock_held(fusefs_rwlock_t *l, krw_t rw)
{
...
/* ARGSUSED */
void
fusefs_rw_init(fusefs_rwlock_t *l, char *name, krw_type_t type, void *arg)
{
...
void
fusefs_rw_destroy(fusefs_rwlock_t *l)
{
