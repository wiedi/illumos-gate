...
#include "xgene-ahcireg.h"
#include "xgene-ahcivar.h"
 ...
 */
static	int ahci_probe(dev_info_t *);
...
static int
ahci_init(ahci_ctl_t *ahci_ctlp)
{
	dev_info_t *dip = ahci_ctlp->ahcictl_dip;
	int status;
	caddr_t reg_addr[REG_NUM] = {0};
	ddi_acc_handle_t acc_handle[REG_NUM] = {0};
	int error = DDI_SUCCESS;

	for (int i = 0; i < REG_NUM; i++) {
		status = ddi_regs_map_setup(dip, i, &reg_addr[i], 0, 0, &accattr, &acc_handle[i]);
		if (status != DDI_SUCCESS && i != REG_MUX) {
			error = DDI_SUCCESS;
			goto out;
		}
	...
	if (reg_addr[REG_MUX]) {
		ddi_put32(acc_handle[REG_MUX], (uint32_t *)(reg_addr[REG_MUX] + 0),
		    ddi_get32(acc_handle[REG_MUX], (uint32_t *)(reg_addr[REG_MUX] + 0)) & ~0x1);
		ddi_get32(acc_handle[REG_MUX], (uint32_t *)(reg_addr[REG_MUX] + 0));
	}

	if (ddi_get32(acc_handle[REG_DIAG], (uint32_t *)(reg_addr[REG_DIAG] + 0x70)) != 0) {
		ddi_put32(acc_handle[REG_DIAG], (uint32_t *)(reg_addr[REG_DIAG] + 0x70), 0);
		drv_usecwait(AHCI_10MS_USECS);
	}

	int num_ports = (ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x00)) & 0x1f);

	for (int i = 0; i < num_ports; i++) {
		// PORTCFG
		ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xa4),
		    (ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xa4)) & ~0x3f) | (i + 2));
		ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xa4));

		// PORTPHY1CFG
		ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xa8), 0x0001fffe);
		ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xa8));

		// PORTPHY2CFG
		ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xac), 0x28183219);
		ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xac));

		// PORTPHY3CFG
		ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xb0), 0x13081008);
		ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xb0));

		// PORTPHY4CFG
		ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xb4), 0x00480815);
		ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xb4));

		// PORTPHY5CFG
		ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xb8),
		    (ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xb8)) & ~(0xfff << 20)) | (0x300 << 20));
		ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xb8));

		// PORTAXICFG
		ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xbc),
		    (ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xbc)) & ~(0xf << 20)) | (0xe << 20) | (1 << 24));
		ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xbc));

		// PORTRANSCFG
		ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xc8),
		    (ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xc8)) & ~0x7f) | 0x30);
		ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0xc8));
	}

	// clear interrupt status
	ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x08), 0xffffffff);
	ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x08));

	// unmask interrupt mask
	ddi_put32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x2c), 0);
	ddi_get32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x2c));

	// unmask error interrupt mask
	ddi_put32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x34), 0);
	ddi_get32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x34));

	// unmask timeout mask
	ddi_put32(acc_handle[REG_AXI], (uint32_t *)(reg_addr[REG_AXI] + 0x10), 0);
	ddi_get32(acc_handle[REG_AXI], (uint32_t *)(reg_addr[REG_AXI] + 0x10));

	// clear error
	ddi_put32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x00), 0xffffffff); // slave read error
	ddi_put32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x04), 0xffffffff); // slave write error
	ddi_put32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x08), 0xffffffff); // master read error
	ddi_put32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x0c), 0xffffffff); // master write error

	// enable coherency
	ddi_put32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x14),
	    (ddi_get32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x14)) & ~0x3f));
	ddi_get32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x14));

	ddi_put32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x18),
	    ddi_get32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x18)) | ((1u << 3) | (1u << 9)));
	ddi_get32(acc_handle[REG_CSR], (uint32_t *)(reg_addr[REG_CSR] + 0x18));

	ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x04),
	    ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x04)) | (1u << 0));

	for (int i = 0;; i++) {
		if (!(ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x04)) & (1u << 0)))
			break;
		if (i >= 100) {
			error = DDI_SUCCESS;
			goto out;
	...
	ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x04), (1u << 31));
	ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x00),
	    (ddi_get32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x00)) & ((1u << 28) | (1u << 17))) | (1u << 27));
	ddi_put32(acc_handle[REG_HOST], (uint32_t *)(reg_addr[REG_HOST] + 0x0c), 0xf);

out:
	for (int i = 0; i < REG_NUM; i++) {
		if (reg_addr[i])
			ddi_regs_map_free(&acc_handle[i]);
	}
...
ahci_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 */
	if (ahci_init(ahci_ctlp) != DDI_SUCCESS)
		goto err_out;
    ...
    ahci_addr_t *addrp, sata_pkt_t *spkt)
{
	...
	switch (ahci_portp->ahciport_lastcmd) {
	case SATAC_ID_DEVICE:
	case SATAC_PACKET:
	case SATAC_SMART:
		if (ahci_software_reset(ahci_ctlp, ahci_portp, addrp) != AHCI_SUCCESS) {
			AHCIDBG(AHCIDBG_ERRS, ahci_ctlp, "softreset error");
			return (AHCI_FAILURE);
		}
	default:
		break;
	...
	ahci_portp->ahciport_lastcmd = scmd->satacmd_cmd_reg;
...
ahci_start_port(ahci_ctl_t *ahci_ctlp, ahci_port_t *ahci_portp, uint8_t port)
{
	...
	ahci_portp->ahciport_lastcmd = 0xFF;
	return (AHCI_SUCCESS);
}
...
ahci_alloc_port_state(ahci_ctl_t *ahci_ctlp, uint8_t port)
{
	dev_info_t *dip = ahci_ctlp->ahcictl_dip;
	ahci_port_t *ahci_portp;
	char taskq_name[64] = "event_taskq";
