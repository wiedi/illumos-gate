...
struct enet_desc_ring {
	...
	uint16_t num;
	bool is_bufpool;
	struct enet_dma desc;
};

struct enet_desc_tx_ring {
	struct enet_desc_ring ring;
	struct enet_dma *dma;
	mblk_t **mblk;
};

struct enet_desc_rx_ring {
	struct enet_desc_ring ring;
...
struct enet_sc {
	...
	struct enet_desc_tx_ring tx_ring;
	struct enet_desc_ring txc_ring;
	struct enet_desc_rx_ring rx_ring;
	...
	int port;
	uint8_t dev_addr[6];
	struct enet_rx_packet *rx_pkt_free;
};
