...
dl_door_call(int door_fd, void *arg, size_t asize, void *rbuf, size_t *rsizep)
{
	...
	return (((dlmgmt_retval_t *)rbuf)->lr_err);
}


/*
 * get the name from dlmgmtd
 */
int
    ...
    dlmgmt_getname_retval_t *name_retval)
{
	...
	return (dl_door_call(door_fd, &getname, sizeof (getname), name_retval,
	    &name_sz) != 0);
}
    ...
    dlmgmt_getnext_retval_t *next_retval)
{
	...
	return (dl_door_call(door_fd, &getnext, sizeof (getnext), next_retval,
	    &next_sz) != 0);
}
...
getallifaddrs(sa_family_t af, struct ifaddrs **ifap, int64_t flags)
{
	...
	if ((door_fd = open(DLMGMT_DOOR, O_RDONLY)) < 0) {
		err = errno;
		close(sock4);
		close(sock6);
		errno = err;
	...
	*ifap = NULL;
	...
	if (af == AF_UNSPEC || af == AF_LINK) {
		...
		while (1) {
			...
			(void) strncpy(lifrl.lifr_name, name_retval.lr_link,
			    sizeof (lifrl.lifr_name));
			...
			struct sockaddr_dl *ifa_addr =  \
			    (struct sockaddr_dl *)curr->ifa_addr;
			if_data_t *ifa_data = curr->ifa_data;
			...
			/*
			 * get metric and interface index
			 * This is only possible if the link has been plumbed.
			 */
			if (ioctl(sock4, SIOCGLIFMETRIC, (caddr_t)&lifrl) \
			    >= 0) {
				ifa_data->ifi_metric = lifrl.lifr_metric;
				if (ioctl(sock4, SIOCGLIFINDEX,
				    (caddr_t)&lifrl) >= 0) {
					ifa_addr->sdl_index = lifrl.lifr_index;
				}
			} else {
				/* retry for IPv6 */
				if (ioctl(sock6, SIOCGLIFMETRIC,
				    (caddr_t)&lifrl) >= 0) {
					ifa_data->ifi_metric = \
					    lifrl.lifr_metric;
				}
				if (ioctl(sock6, SIOCGLIFINDEX,
				    (caddr_t)&lifrl) >= 0) {
					ifa_addr->sdl_index = lifrl.lifr_index;
				}
			}
