 ...
 */
static int
__getifaddrs(struct ifaddrs **ifap, boolean_t can_handle_links)
{
	int		err;
	char		*cp;
	struct ifaddrs	*curr;
 ...
 * this symbol is kept around. Consumers that want the fixed behaviour may
 * recompile and link to the fixed symbol or set the environment variable
 * GETIFADDRS_MAY_RETURN_AF_LINK.
 */
int
getifaddrs(struct ifaddrs **ifap)
{
	boolean_t can_handle_links = false;
	if (getenv("GETIFADDRS_MAY_RETURN_AF_LINK") != NULL)
		can_handle_links = true;

	return (__getifaddrs(ifap, can_handle_links));
}
 ...
 */
int
_getifaddrs(struct ifaddrs **ifap)
{
	return (__getifaddrs(ifap, true));
}
