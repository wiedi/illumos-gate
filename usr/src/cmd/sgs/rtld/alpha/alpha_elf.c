...
/*
 * alpha machine dependent and ELF file class dependent functions.
...
//	printf("%s %s base=%lx enter\n", __func__, lmp->rt_public.l_name, lmp->rt_public.l_addr);

//	__asm__ __volatile__("bpt":::"memory");
	/*
	 ...
	 */
	SLOOKUP_INIT(sl, name, lmp, lml->lm_head, ld_entry_cnt, 0, rsymndx, rsym, 0, LKUP_DEFT);
	...
	if ((lml->lm_tflags | AFLAGS(lmp)) & LML_TFLG_AUD_SYMBIND) {
		uint_t symndx = (((uintptr_t)nsym - (uintptr_t)SYMTAB(nlmp)) / SYMENT(nlmp));
		symval = audit_symbind(lmp, nlmp, nsym, symndx, symval, &sb_flags);
	}
...
elf_reloc(Rt_map *lmp, uint_t plt, int *in_nfavl, APlist **textrel)
{
	 ...
	 */
	if (pltbgn && ((MODE(lmp) & RTLD_NOW) == 0) && (FLAGS(lmp) & FLG_RT_FIXED))
		noplt = 1;
	...
	while (relbgn < relend) {
		 ...
		 */
		if ((rtype == R_ALPHA_RELATIVE) && ((FLAGS(lmp) & FLG_RT_FIXED) == 0) && (DBG_ENABLED == 0)) {
			if (relacount) {
				relbgn = elf_reloc_relative_count(relbgn, relacount, relsiz, basebgn, lmp, textrel, 0);
				relacount = 0;
			} else {
				relbgn = elf_reloc_relative(relbgn, relend, relsiz, basebgn, lmp, textrel, 0);
			}
			if (relbgn >= relend)
				break;
			rtype = ELF_R_TYPE(((Rela *)relbgn)->r_info, M_MACH);
		...
		if (!(FLAGS(lmp) & FLG_RT_FIXED)) {
			 ...
			 */
			if (plthint && (plt == 0) && (rtype == R_ALPHA_JMP_SLOT) && ((MODE(lmp) & RTLD_NOW) == 0)) {
				relbgn = elf_reloc_relative_count(relbgn, plthint, relsiz, basebgn, lmp, textrel, 1);
				plthint = 0;
				continue;
			}
			roffset += basebgn;
		...
		if (rtype == R_ALPHA_NONE)
			continue;
		if (noplt && ((ulong_t)rel >= pltbgn) && ((ulong_t)rel < pltend)) {
			relbgn = pltend;
			continue;
		}
		...
		if ((mpp = find_segment((caddr_t)roffset, lmp)) == NULL) {
			elf_reloc_bad(lmp, (void *)rel, rtype, roffset, rsymndx);
			continue;
		}
		...
		if (rsymndx) {
			 ...
			 */
			symref = (Sym *)((ulong_t)SYMTAB(lmp) + (rsymndx * SYMENT(lmp)));
			...
			if (ELF_ST_BIND(symref->st_info) == STB_LOCAL) {
				 ...
				 * this case exist).
				 */
				if ((rsymndx == psymndx) && (rtype != R_ALPHA_COPY)) {
					/* LINTED */
					if (psymdef == 0) {
						continue;
					}
					...
					if ((LIST(_lmp)->lm_tflags | AFLAGS(_lmp)) & LML_TFLG_AUD_SYMBIND) {
						value = audit_symbind(lmp, _lmp, symdef, dsymndx, value, &sb_flags);
					}
				} else {
					Slookup		sl;
					Sresult		sr;
					...
					SLOOKUP_INIT(sl, name, lmp, 0, ld_entry_cnt, 0, rsymndx, symref, rtype, LKUP_STDRELOC);
					SRESULT_INIT(sr, name);
					symdef = NULL;

					if (lookup_sym(&sl, &sr, &binfo, in_nfavl)) {
						name = (char *)sr.sr_name;
						_lmp = sr.sr_dmap;
						symdef = sr.sr_sym;
					}
					 ...
					 */
					if ((lmp != _lmp) && ((FLAGS1(_lmp) & FL1_RT_NOINIFIN) == 0)) {
						if (aplist_test(&bound, _lmp, AL_CNT_RELBIND) == 0) {
							ret = 0;
							break;
						}
					}
					...
					if (!(FLAGS(_lmp) & FLG_RT_FIXED) && !(IS_SIZE(rtype)) && (symdef->st_shndx != SHN_ABS) && (ELF_ST_TYPE(symdef->st_info) != STT_TLS))
						value += ADDR(_lmp);
					...
					if ((LIST(_lmp)->lm_tflags | AFLAGS(_lmp)) & LML_TFLG_AUD_SYMBIND) {
						dsymndx = (((uintptr_t)symdef - (uintptr_t)SYMTAB(_lmp)) / SYMENT(_lmp));
						value = audit_symbind(lmp, _lmp, symdef, dsymndx, value, &sb_flags);
					}
				}
		 ...
		 * Make sure the segment is writable.
		 */
		if (((mpp->mr_prot & PROT_WRITE) == 0) && ((set_prot(lmp, mpp, 1) == 0) || (aplist_append(textrel, mpp, AL_CNT_TEXTREL) == NULL))) {
			ret = 0;
			break;
		}
		...
		case R_ALPHA_REFQUAD:
			 ...
			 */
			if (do_reloc_rtld(rtype, (uchar_t *)roffset, (Xword *)&value, name, NAME(lmp), LIST(lmp)) == 0)
				ret = 0;
		}

		if ((ret == 0) && ((LIST(lmp)->lm_flags & LML_FLG_TRC_WARN) == 0))
			break;
	}
...
{
	return (conv_reloc_ALPHA_type(rel));
}
