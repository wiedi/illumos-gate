...
elf_reloc(Rt_map *lmp, uint_t plt, int *in_nfavl, APlist **textrel)
{
	...
	while (relbgn < relend) {
		 ...
		 */
		if ((rtype == R_AARCH64_RELATIVE) && ((FLAGS(lmp) & FLG_RT_FIXED) == 0) && (DBG_ENABLED == 0)) {
			if (relacount) {
		...
		if (!(FLAGS(lmp) & FLG_RT_FIXED)) {
			 ...
			 */
			if (plthint && (plt == 0) && (rtype == R_AARCH64_JUMP_SLOT) && ((MODE(lmp) & RTLD_NOW) == 0)) {
		...
		if (rsymndx) {
			...
			if (ELF_ST_BIND(symref->st_info) == STB_LOCAL) {
				 ...
				 * this case exist).
				 */
				if ((rsymndx == psymndx) && (rtype != R_AARCH64_COPY)) {
					/* LINTED */
					if (psymdef == 0) {
						continue;
					}
