...
static void usage();
static int fusefs_unmount(char *, int);
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	/*
	 * Normal users are allowed to umount fusefs mounts they own.
	...
	(void) sprintf(typename, "fusefs %s", myname);
	argv[0] = typename;
	...
	return (fusefs_unmount(argv[optind], umnt_flag));
}
...
{
	(void) fprintf(stderr,
	    gettext("Usage: fusefs umount [-o opts] /dir\n"));
	exit(RET_ERR);
}

static int
fusefs_unmount(char *pathname, int umnt_flag)
{
